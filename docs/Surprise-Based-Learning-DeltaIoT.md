# Surprise-Based Learning (varSMiLE) in DeltaIoT

This document describes how the **Surprise-based learning** approach from the paper “Surprise! Surprise! Learn and adapt” is implemented in DeltaIoT, specifically in `DeltaIOTConnector.updateTransitionBelief()`. The implementation uses a **variational SMiLe (varSMiLE)** rule to update beliefs about the impacts of adaptation actions (POMDP transition probabilities) as a function of how surprising the observed outcomes are.
...
The **varSMiLE** rule in the code is the SMiLe update (Equation 6 in the paper), \(B_{X_n} = (1-\gamma) B_{X_n} + \gamma \pi_0 + \Delta\), with \(B_{X_n}\) and \(\pi_0\) represented as Dirichlet pseudo-counts and \(\Delta\) realised by incrementing those counts before blending (see Section 5).
...
The implementation uses the varSMiLE (Definition 4) formula:
...
## 3. The varSMiLE Update Rule (Step 5–6)
...
- **Role of two arrays**:  
  - `transitionBeliefCurr` is the **learned** belief \(B\); it is updated by the varSMiLE rule and then used in `getTransitionProbability` to define \(W\).  
...
This Dirichlet representation allows **Bayesian** updating (pseudo-count increments) and a **closed-form** blend between current and prior beliefs in the varSMiLE step.

---

## 6. Bayesian Updating: Pseudo-Count Increments for Observed Transitions

The **\(\Delta\)** term in the paper’s SMiLe rule (Equation 6) is implemented by **Bayesian updating** of the Dirichlet pseudo-counts: when the system observes a transition (action \(a\) taken and next state \(s'\) observed), it increments the corresponding counts **before** applying the varSMiLE blend.
...
- **Effect**:  
  ...
  - **Flat prior**: The copy of `transitionBeliefReset` gets the same +1.0; the blended prior in the varSMiLE rule is this “flat + observation” distribution.  
...
5. **varSMiLE**: For the taken action, blend the two temporaries:  
...
────────────────────────────────────────────────────────────────────────────────
STEP 5: Compute adaptation rate γ (varSMiLE formula)
────────────────────────────────────────────────────────────────────────────────
...
────────────────────────────────────────────────────────────────────────────────
STEP 6: varSMiLE update — blend current (observed) and flat (observed) beliefs
────────────────────────────────────────────────────────────────────────────────
